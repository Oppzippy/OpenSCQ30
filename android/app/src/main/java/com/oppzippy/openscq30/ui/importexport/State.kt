...
@Serializable
data class CustomProfile(val name: String, val volumeAdjustments: List<Double>) {
    constructor(dbProfile: StorageCustomProfile) : this(
        dbProfile.name,
        dbProfile.getVolumeAdjustments(),
    )

    fun toStorageCustomProfile(): StorageCustomProfile = StorageCustomProfile(
        name,
        volumeAdjustments[0],
        volumeAdjustments[1],
        volumeAdjustments[2],
...
sealed class ExportCustomProfilesState {
    data class ProfileSelection(val customProfiles: List<CustomProfile>, val selectedProfiles: List<Boolean>) :
    ...
    data class CopyToClipboard(val profileString: String) : ExportCustomProfilesState()
}

sealed class ImportCustomProfilesState {
    data class StringInput(val profileString: String = "", val exception: Exception? = null) :
        ImportCustomProfilesState() {
        fun next(): ImportCustomProfilesState = try {
    ...
    data class ImportOptions(
        ...
        fun getFilteredProfiles(): List<CustomProfile> = profiles
