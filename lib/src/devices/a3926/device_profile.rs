...
pub(crate) const A3926_DEVICE_PROFILE: DeviceProfile = DeviceProfile {
    ...
    compatible_models: &[DeviceModel::SoundcoreA3926],
    implementation: || Arc::new(A3926Implementation::default()),
};

#[derive(Debug, Default)]
struct A3926Implementation {
    buttons: Arc<ButtonConfigurationImplementation>,
}

impl DeviceImplementation for A3926Implementation {
    fn packet_handlers(
        &self,
        ...
        handlers.insert(
            ...
            Box::new(move |packet_bytes, state| {
                let packet = match A3926StateUpdatePacket::take::<VerboseError<_>>(packet_bytes) {
                    Ok((_, packet)) => packet,
                    Err(err) => {
    ...
    fn initialize(&self, packet: &[u8]) -> crate::Result<DeviceState> {
        let packet = A3926StateUpdatePacket::take::<VerboseError<_>>(packet)
            .map(|(_, packet)| packet)
