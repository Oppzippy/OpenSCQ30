...
use crate::{
    ...
    devices::soundcore::common::{
        packet::{outbound::SetSoundModes, packet_io_controller::PacketIOController},
    ...
    ConnectionType: RfcommConnection + Send + Sync,
{
    async fn move_to_state(
        ...
        if needs_noise_canceling {
            ...
            self.packet_io
                .send_with_response(&SetSoundModes(new_sound_modes).into())
                .await?;
        ...
        if needs_ambient_sound_mode_revert {
            self.packet_io
                .send_with_response(&SetSoundModes(*target_sound_modes).into())
                .await?;
