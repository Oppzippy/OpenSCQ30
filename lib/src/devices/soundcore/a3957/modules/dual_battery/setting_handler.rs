    ...
    fn get(&self, state: &T, setting_id: &SettingId) -> Option<Setting> {
        ...
        Some(match setting {
            ...
            BatterySetting::BatteryLevelLeft => Setting::Information {
                value: format!("{}/{}", battery.left.level.0 + 1, self.max_level),
                translated_value: fl!(
                    "percent",
                    percent =
                        ((i32::from(battery.left.level.0 + 1) * 100) / i32::from(self.max_level))
                ),
            },
            BatterySetting::BatteryLevelRight => Setting::Information {
                value: format!("{}/{}", battery.right.level.0 + 1, self.max_level),
                translated_value: fl!(
                    "percent",
                    percent =
                        ((i32::from(battery.right.level.0 + 1) * 100) / i32::from(self.max_level))
                ),
            },
        })
    }
