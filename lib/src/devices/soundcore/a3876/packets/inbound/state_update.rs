...
#[async_trait]
impl PacketHandler<A3876State> for StateUpdatePacketHandler {
    async fn handle_packet(
        &self,
        state: &watch::Sender<A3876State>,
        packet: &packet::Inbound,
    ) -> device::Result<()> {
        let packet: A3876StateUpdatePacket = packet.try_to_packet()?;
        state.send_modify(|state| state.update(packet));
        Ok(())
    }
}

impl ModuleCollection<A3876State> {
