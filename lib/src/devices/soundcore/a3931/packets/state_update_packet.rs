...
impl IntoPacket for A3931StateUpdatePacket {
    type DirectionMarker = packet::InboundMarker;
...
impl PacketHandler<A3931State> for StateUpdatePacketHandler {
    async fn handle_packet(
        ...
        let packet: A3931StateUpdatePacket = packet.try_into_packet()?;
...
mod tests {
    ...
    fn serialize_and_deserialize() {
        let bytes = A3931StateUpdatePacket::default().into_packet().bytes();
        let (_, packet) = packet::Inbound::take::<VerboseError<_>>(&bytes).unwrap();
        let _: A3931StateUpdatePacket = packet.try_into_packet().unwrap();
    }
}
