...
) -> SoundcoreDeviceRegistry<B, A3028Device<B::ConnectionType>> {
...
pub fn demo_device_registry(
    ...
    A3028Device<<DemoConnectionRegistry as RfcommBackend>::ConnectionType>,
> {
...
pub struct A3028Device<ConnectionType: RfcommConnection + Send + Sync> {
    device_model: DeviceModel,
    state_sender: watch::Sender<A3028State>,
    module_collection: Arc<ModuleCollection<A3028State>>,
    _packet_io_controller: Arc<PacketIOController<ConnectionType>>,
}

impl<ConnectionType> SoundcoreDevice<ConnectionType> for A3028Device<ConnectionType>
where
    ConnectionType: RfcommConnection + 'static + Send + Sync,
{
    async fn new(
        ...
        let state_update_packet: A3028StateUpdatePacket = packet_io_controller
...
#[async_trait]
impl<ConnectionType> OpenSCQ30Device for A3028Device<ConnectionType>
where
    ConnectionType: RfcommConnection + 'static + Send + Sync,
{
