...
impl RfcommBackend for WindowsRfcommBackend {
    ...
    async fn connect(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let services = device.GetRfcommServicesAsync()?.get()?.Services()?;
...
impl WindowsRfcommBackend {
    ...
    fn get_bluetooth_device_from_mac_address(
        ...
        let device_information_collection =
            DeviceInformation::FindAllAsyncAqsFilter(&filter)?.get()?;
        trace!(
        ...
        Ok(BluetoothDevice::FromIdAsync(&device_information.Id()?)?.get()?)
    }
}
